...
add_action( 'wp_enqueue_scripts', 'cf_enqueue_scripts', 100 );

// Замена заголовка "Платёжный адрес" на "Контакты" на странице благодарности и в письмах
function cf_replace_billing_address_title($title, $type) {
    if ($type === 'billing') {
        return 'Контакты';
    }
    return $title;
}
add_filter('woocommerce_my_account_my_address_title', 'cf_replace_billing_address_title', 10, 2);
add_filter('woocommerce_order_get_formatted_billing_address', 'cf_replace_billing_address_title', 10, 2);
add_filter('woocommerce_email_billing_address_title', 'cf_replace_billing_address_title', 10, 2);
...
function cf_add_custom_checkout_field( $checkout ) {
    ...
    echo '<input type="hidden" name="cf_lift_cost" value="0">'; 
...
// Добавление информации о количестве этажей в таблицу "Информация о заказе"
if ( ! function_exists( 'cf_add_lift_info_to_review_order' ) ) {
    function cf_add_lift_info_to_review_order() {
        $lift_option = WC()->session->get('cf_lift_option', 'no_lift');
        $floor_count = WC()->session->get('cf_floor_count', '0');

        // Если выбран "Требуется подъем на этаж, грузового лифта нет", то отображаем количество этажей
        if ($lift_option === 'lift') {
            ?>
            <tr class="cf-floor-count">
                <th><?php _e( 'Количество этажей:', 'woocommerce' ); ?></th>
                <td class="cf-floor-count-label"><?php echo esc_html( $floor_count ); ?></td>
            </tr>
            <?php
        }
    }
}
add_action( 'woocommerce_review_order_before_order_total', 'cf_add_lift_info_to_review_order', 10 );

// AJAX обработка для получения значений кастомных полей
function cf_get_lift_info() {
    $lift_option = WC()->session->get('cf_lift_option', 'no_lift');
    $floor_count = WC()->session->get('cf_floor_count', '0');

    wp_send_json_success(array(
        'lift_option' => $lift_option,
        'floor_count' => $floor_count,
    ));
}
add_action('wp_ajax_cf_get_lift_info', 'cf_get_lift_info');
add_action('wp_ajax_nopriv_cf_get_lift_info', 'cf_get_lift_info');
